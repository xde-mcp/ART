...
ART uses GRPO (a reinforcement learning technique published by DeepSeek in 2024) is used to train LLMs (large language models) to more reliably follow instructions provided in their system and user messages. By allowing an LLM to make multiple attempts at accomplishing a task and scoring each rollout’s performance, we shift the LLM’s weights to make it more likely to perform the way it did in its best runs while teaching it to avoid its worst behavior.
