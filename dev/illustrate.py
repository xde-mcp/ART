...
def illustrate(
    ...
    gradients = {
        ...
        "one-dark-simple": [
            ...
            (0.35, (10, 0.30, 0.60)),  # Subtle warm transition (less orange)
            (0.5, (220, 0.08, 0.70)),  # Nearly neutral gray with minimal warm tint
    ...
    def interpolate_hsl(val, gradient_stops, gradient_name):
        ...
        # Apply non-linear transformation for one-dark-simple to accelerate towards extremes
        if gradient_name == "one-dark-simple":
            # Apply a power curve that keeps middle values gray longer
            # and accelerates towards red/blue at extremes
            if t < 0.5:
                # For negative values, apply stronger curve
                t = 0.5 * (2 * t) ** 2.5
            else:
                # For positive values, apply stronger curve
                t = 1 - 0.5 * (2 * (1 - t)) ** 2.5
    ...
    # Add a gradient legend at the bottom
    result_parts.append("\n\033[90m" + "─" * 60 + "\033[0m\n")
    result_parts.append(f"Gradient ({gradient}): ")

    # Show a sample of the gradient
    for i in range(20):
        t = (i / 19) * 6 - 3  # Map to [-3, 3]
        color = interpolate_hsl(t, gradient_stops, gradient)
        result_parts.append(f"{color}█\033[0m")
