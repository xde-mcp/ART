def illustrate(tokens_and_values: list[tuple[str, float]]) -> str:
    """
    Returns a renderable string concatenating the tokens colored by their values in an aesthetically pleasing way.

    Note: Returns renderable string compatabile with the "One Dark Pro" VSCode theme.
    ...
    Returns:
        A renderable string compatabile with the "One Dark Pro" VSCode theme.
    """
    import numpy as np
    ...
    # Using ANSI escape codes for terminal coloring
    def value_to_color(val):
        # Map normalized values to color intensity
        # Using a gradient from red (negative) through gray (neutral) to green (positive)
        if val < -2:
            return "\033[38;2;224;108;117m"  # Bright red (One Dark Pro error color)
        elif val < -1:
            return "\033[38;2;190;80;70m"  # Red
        elif val < -0.5:
            return "\033[38;2;209;154;102m"  # Orange (One Dark Pro warning color)
        elif val < 0.5:
            return "\033[38;2;171;178;191m"  # Gray (One Dark Pro default text)
        elif val < 1:
            return "\033[38;2;152;195;121m"  # Light green (One Dark Pro string color)
        elif val < 2:
            return "\033[38;2;134;175;106m"  # Green
        else:
            return "\033[38;2;97;175;239m"  # Blue (One Dark Pro function color)

    # Build the output string with colored tokens
    result_parts = []

    # Add header with value range info
    min_val = values.min()
    max_val = values.max()
    mean_val = values.mean()
    std_val = values.std() if len(values) > 1 else 0

    # Format the stats line with proper padding
    stats_line = f"Stats: μ={mean_val:6.3f}, σ={std_val:6.3f}, min={min_val:6.3f}, max={max_val:6.3f}"
    # Calculate padding needed to center and align the content
    box_width = 63  # Total width including borders
    content_width = box_width - 4  # Subtract 2 for "║ " and " ║"
    padded_stats = stats_line.center(content_width)

    result_parts.append(f"\033[90m╔{'═' * (box_width - 2)}╗\033[0m\n")
    result_parts.append(f"\033[90m║ {padded_stats} ║\033[0m\n")
    result_parts.append(f"\033[90m╚{'═' * (box_width - 2)}╝\033[0m\n\n")

    # Add the colored tokens
    ...
    result_parts.append("Value Distribution:\n")

    # Find the maximum token length for alignment
    ...
    for token, orig_val, norm_val in zip(tokens, values, normalized_values):
        ...
        padded_token = token.ljust(max_token_len)

        # Create the bar
        bar_width = 20
        bar_position = int(
            (norm_val + 3) / 6 * bar_width
        )  # Map from [-3, 3] to [0, bar_width]
        bar_position = max(0, min(bar_width - 1, bar_position))

        # Build the bar
        bar = ""
        for i in range(bar_width):
            if i == bar_width // 2:  # Center line
                bar += "│"
            elif i == bar_position:
                color = value_to_color(norm_val)
                bar += f"{color}█\033[0m"
        ...
        color = value_to_color(norm_val)
        result_parts.append(
            f"  {color}{padded_token}\033[0m  [{bar}]  {orig_val:7.3f}\n"
        )

    # Add a legend at the bottom
    result_parts.append("\n\033[90m" + "─" * 60 + "\033[0m\n")
    result_parts.append("Legend: ")
    result_parts.append("\033[38;2;224;108;117m█\033[0m Very Negative  ")
    result_parts.append("\033[38;2;209;154;102m█\033[0m Negative  ")
    result_parts.append("\033[38;2;171;178;191m█\033[0m Neutral  ")
    result_parts.append("\033[38;2;152;195;121m█\033[0m Positive  ")
    result_parts.append("\033[38;2;97;175;239m█\033[0m Very Positive")
...
theme = {
    ...
    "semanticTokenColors": {
        ...
        "memberOperatorOverload": {"foreground": "#c678dd"},
    ...
    "tokenColors": [
        ...
        {"scope": ["log.info"], "settings": {"foreground": "#98c379"}},
        {"scope": ["log.warning"], "settings": {"foreground": "#e5c07b"}},
        {"scope": ["log.error"], "settings": {"foreground": "#e06c75"}},
        {
            "scope": "keyword.operator.expression.is",
            "settings": {"foreground": "#c678dd"},
        },
        {"scope": "entity.name.label", "settings": {"foreground": "#e06c75"}},
        {
            "scope": [
